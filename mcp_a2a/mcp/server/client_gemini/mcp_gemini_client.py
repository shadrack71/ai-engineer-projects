...
class MCPClient:
    ...
    async def connect_to_server(self, server_script_path: str):
        ...
        # 2. Use the exact same Python interpreter
        command = sys.executable if server_script_path.endswith('.py') else "node"

        # 3. CRITICAL WINDOWS FIX: Force UTF-8 and disable I/O buffering
        env = os.environ.copy()
        env["PYTHONUTF8"] = "1"
        env["PYTHONIOENCODING"] = "utf-8"
        env["PYTHONUNBUFFERED"] = "1"

        # Define the parameters, explicitly passing our modified environment
